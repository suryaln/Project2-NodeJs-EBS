pipeline {
    ...
    environment {
        ...
        AWS_ACCESS_KEY_ID = credentials('AKIA27ON736KJLY5E3A7')
        AWS_SECRET_ACCESS_KEY = credentials('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')
        APPLICATION_NAME = 'Nodejs-web'
        ENVIRONMENT_NAME = 'Nodejs-web-env'
        S3_BUCKET = 'elasticbeanstalk-ap-south-1-754735636372' // Give your s3 name created by aws beanstalk
    ...
    stages {
       stage('Checkout') {
            steps {
                ...
                checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/suryaln/Project2-NodeJs-EBS.git']])
            }
        }
